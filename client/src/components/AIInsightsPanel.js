...
export default function AIInsightsPanel({ products = [], activeTab, searchQuery, selectedStage, sortBy }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white/90 dark:bg-slate-900/90 shadow-xl p-5 sm:p-6">
      <div className="flex items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-2">
          <FaBrain className="text-blue-600 dark:text-blue-400" />
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white">AI Dashboard Insights</h3>
        </div>
        <button
          type="button"
          onClick={handleGenerate}
          disabled={!aiEnabled || !canGenerate || loading}
          className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed"
        >
